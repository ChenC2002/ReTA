...
Expected input (CSV/Parquet) at the event level (one row per diagnosis event):
...
try:
    from .ontology import CCSOntology
except ImportError:  # allow running as a script
    from ontology import CCSOntology  # type: ignore
...
def make_24h_visit_bins(df: pd.DataFrame, patient_col: str, time_col: str) -> pd.Series:
    """Assign a visit_bin id by 24-hour aggregation.

    For each patient, sort by time, then compute bin boundaries where time gap >= 24h.
    Returns an integer series aligned with df.
    """
    ...
    for i, (pid, ts) in enumerate(zip(df[patient_col].values, df[time_col].values)):
        if last_pid is None or pid != last_pid:
            ...
            if pd.isna(ts) or pd.isna(last_time):
                # treat missing times as new bin to avoid leakage
                cur += 1
            else:
                gap_h = (ts - last_time) / np.timedelta64(1, 'h')
                if gap_h >= 24.0:
                    cur += 1
            last_time = ts
        bins[i] = cur
...
@dataclass
class VisitRecord:
    icd_codes: List[int]               # ICD token ids
    ccs_ancestors: List[int]           # CCS ancestor token ids (including direct CCS)
    node_ids: List[int]               # node ids used in visit graph
    edge_index: np.ndarray            # shape (2, E) for PyG
    label_ccs: Optional[List[int]] = None  # CCS label ids for next visit
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--events', type=str, required=True, help='Path to event-level diagnoses (CSV/Parquet).')
...
def main():
    ...
    ontology = CCSOntology.from_files(args.icd2ccs, args.ccs_hierarchy)
    preprocess(
