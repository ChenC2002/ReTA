...
def project_to_dim(X: np.ndarray, dim: int) -> np.ndarray:
    """Project template embeddings into the model embedding space.

    Template vectors ``p_k`` live in ``R^d``. Sentence encoders and TF-IDF
    fallbacks may produce a different dimensionality, so we use a deterministic
    projection before computing template centroids.
    """
    dim = int(dim)
    if dim <= 0:
...
class TextEmbedder:
    ...
    def _init_backend(self):
        ...
        try:
            ...
            self.backend = AutoModel.from_pretrained(self.model_name)
            self.backend.eval()
            self.backend_kind = "transformer"
            return
        except Exception:
            self.backend = None
            self.backend_kind = None
...
def cluster_templates(
    ...
    X = project_to_dim(embedder.encode(texts), projection_dim)
    ...
    if len(grounded) == 1:
        return [
            KnowledgeTemplate(
                template_id=0,
                vector=Xn[0].astype(float).tolist(),
                medoid_idx=0,
                medoid=grounded[0],
                member_indices=[0],
            )
...
def main():
    ...
    templates = cluster_templates(grounded, tau=args.tau, model_name=args.model_name, projection_dim=args.projection_dim)
