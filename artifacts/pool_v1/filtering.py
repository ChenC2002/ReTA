"""Deterministic grounding and external-support filtering for ``pool_v1``.

This module intentionally does not call an embedding model.  The paper's
ClinicalBERT fallback must be computed separately and supplied as one Top-1
candidate per mention.  That keeps the release filter deterministic and makes
the model output auditable.

Input contracts
---------------
``artifacts_jsonl`` follows ``schemas/distilled_artifact.schema.json``.
...
class Inventory:
    ...
    def __init__(self, entities: Iterable[Entity]):
        ...
        for raw in entities:
            entity = Entity(_clean_id(raw.entity_id), str(raw.name).strip(), str(raw.source or "unknown").strip())
    ...
    def get(self, entity_id: str) -> Optional[Entity]:
        return self._by_id.get(_clean_id(entity_id))
...
class SupportIndex:
    ...
    def __init__(
        ...
        for parent, child in ccs_edges:
            parent, child = _clean_id(parent), _clean_id(child)
    ...
    def find_support(self, root: str, target: str) -> SupportDecision:
        ...
        down = _shortest_directed_path(self._parent_to_children, root, target, MAX_CCS_HOPS)
        if down is not None:
            paths.append((len(down) - 1, down, "root_is_ancestor"))
        up = _shortest_directed_path(self._child_to_parents, root, target, MAX_CCS_HOPS)
        if up is not None:
...
def _filter_valid_artifact(
    ...
    graph_nodes: Set[str] = {record.concept_id}
    ...
    for index, mention in enumerate(record.cascade):
        ...
        support_decision = support.find_support(record.concept_id, mapping.entity.entity_id)
    ...
    reachable = _connected_component(record.concept_id, graph_edges)
    nodes = [record.concept_id] + sorted((graph_nodes & reachable) - {record.concept_id})
    edges = sorted(
        edge
    ...
    cascade_entities = []
    for entity_id in sorted(accepted_by_entity):
    ...
    grounded = {
        "root_concept_id": record.concept_id,
