...
- node_ids (N,)
- edge_index (2,E)
- orig_mask (N,) for nodes present before Hard Import
- code_mask (N,) for observed visit ICD codes; falls back to orig_mask for older data
"""
...
class DecoupledMultiGATEncoder(nn.Module):
    ...
    def _effective_code_mask(code_mask: torch.Tensor, orig_mask: torch.Tensor, batch: torch.Tensor) -> torch.Tensor:
        """Prefer observed ICD nodes, falling back per graph when old records lack them."""
    ...
    def forward(
        ...
        """
        Returns:
        - h_G: (B,d) pooled visit embeddings (sum over observed visit codes)
        ...
        # Optional Soft Import (batched): add xi * φ(p_k) to observed visit codes.
        if soft_offset_per_graph is not None:
            ...
            if soft_offset_per_graph.shape[1] != self.dim:
                raise ValueError(
                    f"soft_offset_per_graph shape {soft_offset_per_graph.shape}, expected second dimension {self.dim}"
                )
        ...
        # Pool: sum over observed visit codes only (Eq.13)
        B = int(batch.max().item()) + 1 if batch.numel() > 0 else 0
