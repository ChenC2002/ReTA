...
class SemanticEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch: torch.Tensor, orig_mask: torch.Tensor) -> torch.Tensor:
        """Return h_sem (N,d). Non-original nodes -> 0."""
        _require_pyg()
        ...
        x_dense, mask = to_dense_batch(x0, batch=batch)  # (B,T,d), (B,T)
