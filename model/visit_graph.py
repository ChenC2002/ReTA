...
def build_pyg_data(node_ids: List[int], edge_index: torch.Tensor, orig_mask: Optional[torch.Tensor] = None):
    _require_pyg()
    ...
    return Data(node_ids=node_ids_t, edge_index=edge_index, orig_mask=orig_mask)
...
def from_sample_dict(sample: Dict) -> "Data":
    ...
    orig_mask = torch.ones(len(node_ids), dtype=torch.bool)
    return build_pyg_data(node_ids, edge_index, orig_mask=orig_mask)
